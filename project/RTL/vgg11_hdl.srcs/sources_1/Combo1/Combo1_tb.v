...
module Combo1_tb();
  ...
  //------------------
	...
	//integer pixel_data, pixel_red_in,  pixel_result, i;
	
	integer i;
	integer pixel_red_in, pixel_green_in, pixel_blue_in;
	integer result_map0;
	
	
	initial begin
		pixel_red_in = $fopen("....path of red map 224 x 224....", "r");
    pixel_green_in = $fopen("....path of green map 224 x 224....", "r");
    pixel_blue_in = $fopen("....path of blue map 224 x 224....", "r");
		result_map0 = $fopen("....path of result....", "w");
    ...
    #5
		...
		for(i=0; i <= num_step; i=i+1) begin
			$fscanf(pixel_red_in, "%b\n", red_in);
			$fscanf(pixel_green_in, "%b\n", green_in);
			$fscanf(pixel_blue_in, "%b\n", blue_in);
			#clk_prd
			if(i == num_step) $finish;
		end
		
		$fclose(pixel_red_in);
		$fclose(pixel_green_in);
		$fclose(pixel_blue_in);

	end
	always @(posedge clk) begin
      $fdisplay(result_map0, "%b" , out0);
	end
